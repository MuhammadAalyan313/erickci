...
//search location
$(document).on("input paste click", "#input_location_modal", function () {
    var input_value = $(this).val();
    if (input_value.length > 0) {
        $('.btn-reset-location-input_modal').show();
    } else {
        $('#location_id_inputs_modal input').val('');
        $('.btn-reset-location-input_modal').hide();
    }
    if (input_value.length < 2) {
        $('#response_search_location_modal').hide();
        return false;
    }
    var data = {
        "input_value": input_value,
    ...
    $.ajax({
        ...
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
                document.getElementById("response_search_location_modal").innerHTML = obj.response;
                $('#response_search_location_modal').show();
            }
            //search text
            $('#response_search_location_modal ul li a').wrapInTag({
                words: [input_value]
            });
        }
    });
...
$('#input_location_modal').on('focus', function () {
    $("#search-results-ajax_modal").show();
});

//set location
$(document).on("click", "#response_search_location_modal ul li a", function () {
    // $('#input_location').val($(this).text());
    // var country_id = $(this).attr('data-country');
    // var state_id = $(this).attr('data-state');
    // var city_id = $(this).attr('data-city');
...
$(document).on('click', '#btn_submit_location_modal', function () {
    var data = {
        "country_id": $("#location_id_inputs_modal input[name='country']").val(),
        "state_id": $("#location_id_inputs_modal input[name='state']").val(),
        "city_id": $("#location_id_inputs_modal input[name='city']").val(),
        "sys_lang_id": mds_config.sys_lang_id
    };
    data[mds_config.csfr_token_name] = $.cookie(mds_config.csfr_cookie_name);
    $.ajax({
...
$(document).on('click', '.btn-reset-location-input_modal', function () {
    $('#input_location_modal').val('');
    $('#location_id_inputs_modal input').val('');
    $(this).hide();
});
...
if ($(".profile-cover-image")[0])
{
    ...
    document.addEventListener('lazybeforeunveil', function (e) {
        ...
        var bg = $(".profile-cover-image")[0].getAttribute('data-bg-cover');
        //console.log("here even")
        if (bg) {
            $(".profile-cover-image")[0].style.backgroundImage = 'url(' + bg + ')';
        }
    });
}
...
$(document).on("change", "#select_sort_items", function () {
    ...
    var current_url = $(this).attr('data-current-url');
    var page = $(this).attr('data-page');
    if (val == "most_recent" || val == "lowest_price" || val == "highest_price" || val == "negotiable") {
        var params = "sort=" + val;
        if (query_string == "") {
            query_string = "?" + params;
        } else {
        ...
        success: function (response) {
            ...
            if (obj.result == 1) {
                ...
                setTimeout(() => {
                    ...
                    document.getElementById("response_message").innerHTML = obj.html_content;
                    console.log("messaga succss");
                }, 100);
                setTimeout(() => {
                ...
                setTimeout(() => {
                    $('#fail_pop').modal('show');
                    document.getElementById("fail_response_message").innerHTML = obj.html_content;
                    console.log("messaga fial");
                }, 100);
                setTimeout(() => {
                    $('#fail_pop').modal('hide');
                    document.getElementById("fail_response_message").innerHTML = obj.html_content;
            ...
            if (message_send_em == 1) {
                ...
                // document.getElementById("response_email").innerHTML = obj.html_content;
            }
            // else{
            //     document.getElementById("fail_response_email").innerHTML = obj.html_content;
            // }
        }
    });
});
...
function add_fav_conversation(conversation_id) {
    ...
    $.ajax({
        method: "POST",
        url: mds_config.base_url + "message_controller/add_fav_conversation",
        data: data
    })
    .done(function (response) {
        location.reload();
...
function delete_fav_conversation(conversation_id) {
    ...
    $.ajax({
        method: "POST",
        url: mds_config.base_url + "message_controller/delete_fav_conversation",
        data: data
    })
    .done(function (response) {
        location.reload();
...
document.getElementById("collapse-button").addEventListener('click',(e)=>{
    document.getElementById('shop-list-container').classList.toggle('shop-list-container-collapsed')
    document.getElementById('collapse-arrow').classList.toggle('button-expand')
})

let shareButtons = document.querySelectorAll(".share-btn")
shareButtons.forEach(sharebtn => {sharebtn.addEventListener('click', async (e) => {
    e.stopPropagation();
    const urlToShare = e.currentTarget.getAttribute("data-url");
    const shopName = e.currentTarget.getAttribute("data-name");
    try {
...
const directionsButton = document.querySelectorAll('.directionsButton');

directionsButton.forEach(directionBtn => {directionBtn.addEventListener('click', (e) => {
    e.stopPropagation();
  const address = e.currentTarget.getAttribute("data-address"); // Replace with the desired address
...
follow_unfollow_buttons = document.querySelectorAll(".follow_unFollow_btn")

follow_unfollow_buttons.forEach(btn => btn.addEventListener('click', (e)=>{
    e.stopPropagation();
    follow_unfollow_submit(e) 
})
)
function follow_unfollow_submit(e) {
    auth_userId = e.currentTarget.getAttribute("auth-user");
    user = e.currentTarget.getAttribute("shop");
    
    // let shop_id = element.getAttribute('shop')
    element =  e.currentTarget
    
    openCoversationContactButtons(user)
  }
const spanele = document.querySelectorAll("#shop-buttoms > span")
console.log(spanele)
// spanele.forEach(ele => {
...
//     })
// });
// let atags = document.querySelectorAll(".shop-actions-buttons > a")
// atags.forEach(atag => {
