...
class Home_controller extends Home_Core_Controller
{
    public function __construct()
    {
        ...
        $this->location_model->get_location_ip();
    }
    ...
    public function index()
    {
        ...
        $email = array(
            ...
            'receiver' => 'testingserver95@gmail.com',
    ...
    public function products($slug = null)
    {
        ...
        if ($slug != null) {
            //get paginated posts
            $data['breadcrumb'] = trans("is_negotiable");
            $pagination = $this->paginate(generate_url('negotiable'), $this->product_model->get_paginated_filtered_products_count($data['query_string_array'], 'negotiable', $data['custom_filters']), $this->product_per_page);
            $data['products'] = $this->product_model->get_paginated_filtered_products($data['query_string_array'], 'negotiable', $data['custom_filters'], $pagination['per_page'], $pagination['offset']);
        } else {
            //get paginated posts
            $data['breadcrumb'] = trans("products");
    ...
    public function product($slug)
    {
        ...
        if (empty($data['product'])) {
            ...
            $data['nexturl'] =   $this->nextUrl($data["product"]->id);
            $data['previousurl'] =   $this->previousUrl($data["product"]->id);
    ...
    public function shops()
    {
        ...
        // var_dump($this->license_model->get_allowed_licenses());
        // die;
        $licenses_allowed = '';
        if ($licenseRisData->allowed_id != "null") {
            $licenses_allowed = implode(',', json_decode($licenseRisData->allowed_id));
        }

        $query = "";
        if ($is_not_restricted) {
            if (empty($licenses_allowed)) {
                $query = 'AND u.license_type =0 OR u.license_type = null';
            } else {
                $query = "AND u.license_type in ($licenses_allowed)";
            }
        }
        ...
        if ($page->visibility == 0) {
            ...
            $data['keywords'] = $page->keywords;
            $data['page'] = $page;
            $data['auth_user_location'] = "not_set";
            if ($this->auth_check && $this->auth_user->address != Null && $this->auth_user->address != '') :
                $data['auth_user_location'] = get_location($this->auth_user);
            endif;
        ...
        FROM
            `users` u
        LEFT JOIN
            `location_cities` ci ON u.`city_id` = ci.`id`
        LEFT JOIN
            `location_states` s ON u.`state_id` = s.`id`
        LEFT JOIN
            `location_countries` c ON u.`country_id` = c.`id`
        WHERE
            (u.`role_id` = 1 OR u.`role_id` = 2)  AND (u.`address` IS NOT NULL AND u.`address` <> '') AND has_active_shop = 1  " . $query)->result();
            // $result = $this->db->result();
            foreach ($result as $res) {
                if ($this->auth_check) :
                    $res->follows = is_user_follows($res->id, $this->auth_user->id);
                endif;
                $res->isOnline = is_user_online($res->last_seen);
                $res->rating = calculate_user_rating($res->id);
                if ($res->acc_type == 3) {
                    $res->verify = ' class="icon-verified icon-verified-member" style="font-size: 20px"';
                } elseif ($res->acc_type == 2) {
                    $res->verify = 'class="icon-verified icon-verified-member" style="color:orange;font-size: 20px;"';
                } else {
                    $res->verify = 'class="display-none"';
                }
            }
            // echo"<pre>";
            // var_dump($data['auth_user_location']);
            // echo"</pre>";
            // die( );
            $data['session'] = $this->session->userdata('mds_default_location');
            $data['results'] = $result;
    ...
    {
        get_method();
        $location = $this->location_model->get_default_location();
        $location_query = '';
        $zero = 0;
        if ($location->country_id) {
            $location_query .= " AND u.`country_id` = " . $location->country_id;
        }
        if ($location->state_id) {
            $location_query .= " AND u.state_id = " . $location->state_id;
        }
        if ($location->city_id) {
            $location_query .= " AND (u.city_id = " . $location->city_id . " OR u.city_id = 0)";
        }
        // echo $location_query;
        if (!is_multi_vendor_active()) {
            redirect(lang_base_url());
        ...
        LEFT JOIN
            `location_countries` c ON u.`country_id` = c.`id`
        WHERE
            (u.`role_id` = 1 OR u.`role_id` = 2) AND (u.`address` IS NOT NULL AND u.`address` <> '') AND has_active_shop = 1
            $location_query")->result();
            // $result = $this->db->result();
            foreach ($result as $res) {
                if ($this->auth_check) :
                    $res->follows = is_user_follows($res->id, $this->auth_user->id);
    ...
    function get_mapLocation()
    {
        $swLat = $this->input->post("swLat");
        $neLat = $this->input->post("neLat");
        $neLng = $this->input->post("neLng");
        $swLng = $this->input->post("swLng");
        $location_query = " AND u.`country_id` = " . "230";

        // if ($state && $city) {
        //     $state_id = $this->location_model->get_state_from_ip($state);
        //     $city_id = $this->location_model->get_city_from_ip($state_id->id, $city);
        //     $location_query .= " AND u.state_id = " .  $state_id->id . " AND (u.city_id = " . $city_id->id . " OR u.city_id = 0)";
        //     // echo json_encode(["sate" => $state_id, "city" => $city_id, "ASdf" => $state, "custs" => $city]);
        // } elseif ($state) {

        //     $state_id = $this->location_model->get_state_from_ip($state);
        //     $location_query .= " AND u.state_id = " .  $state_id->id;
        //     // echo json_encode(["sate" => $state_id]);
        // }
        $is_not_restricted =  $this->license_model->get_license_restriction_setting();
        $licenseRisData = $this->license_model->get_allowed_licenses();
        $licenses_allowed = implode(',', json_decode($licenseRisData->allowed_id));
...
(
  ...
  OR
  (lng <= $swLng AND lng >= $neLng)
) AND  (u.`role_id` = 1 OR u.`role_id` = 2) AND has_active_shop = 1 " . $query)->result();
        // $result = $this->db->result();
        foreach ($result as $res) {
            if ($this->auth_check) :
                $res->follows = is_user_follows($res->id, $this->auth_user->id);
        ...
        $data['results'] = $result;
        // echo json_encode(["swLat" =>$swLat ,"neLat"=>$neLat,"neLng"=>$neLng,"swLng"=>$swLng]);
        echo json_encode(["swLat" => $swLat, "neLat" => $neLat, "neLng" => $neLng, "swLng" => $swLng, "data" => $result]);
        return 0;
    }
    /**
     * map
    ...
    public function shops_map()
    {
        ...
        LEFT JOIN
            `location_countries` c ON u.`country_id` = c.`id`
        WHERE
            (u.`role_id` = 1 OR u.`role_id` = 2)AND (u.`address` IS NOT NULL AND u.`address` <> '')")->result();
            // $result = $this->db->result();
            foreach ($result as $res) {
                if ($this->auth_check) :
                    $res->follows = is_user_follows($res->id, $this->auth_user->id);
    ...
    public function start_selling()
    {
        ...
        $data['rejection_reasons'] = $this->membership_model->get_decline_reason($this->auth_user->id)->row();
    ...
    public function start_selling_post()
    {
        ...
        $address = $this->input->post('address', true);
        $this->location_model->set_coordinates( $this->auth_user->id, $state, $city, $address);
        $email = array(
            'sender_email' => '',
            'sender_name' => 'Test Email',
            'receiver' => 'testingserver95@gmail.com',
    ...
    */
    ...
    public function add_review_post()
    {
        if ($this->auth_check && $this->general_settings->reviews == 1) {
            ...
            if ($product->user_id != $this->auth_user->id) {
                ...
                $this->notification_model->reviews_notification($data);
                if (!empty($review)) {
    ...
    public function nextUrl($productId)
    {
        if ($this->product_model->NextAd($productId)) {
            $url = generate_product_url($this->product_model->NextAd($productId));
            return $url;
        }
        return "end";
    }
    public function previousUrl($productId)
    {
        if ($this->product_model->previousAd($productId)) {
            $url = generate_product_url($this->product_model->previousAd($productId));
            return $url;
        }
        return "end";
    }
    ...
    {
        unset($_SESSION['mds_default_location']);
        $this->location_model->get_location_ip();
        redirect($this->agent->referrer());
    }
