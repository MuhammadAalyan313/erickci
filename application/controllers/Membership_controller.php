...
class Membership_controller extends Admin_Core_Controller
{
    ...
    public function getAllUsers()
    {
        ...
        $pagination = $this->paginate($data['page_url'], $this->auth_model->get_users_all_count('admin'));
        $data['users'] = $this->auth_model->get_paginated_all_users( $pagination['per_page'], $pagination['offset']);
        $data['roles'] = $this->membership_model->get_roles();
        $data['acc_type'] = $this->membership_model->get_acc_type();

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/membership/users');
    ...
    public function edit_user_post()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if( $this->input->post('license_number', true)!= ""){
                if($this->input->post('license_type', true)== ""){
                    $this->session->set_flashdata('license_data',  $this->input->post('license_number', true));
                    // $this->session->set_flashdata('license_data',  null);
                    $this->session->set_flashdata('error', "License Type is not selected");
                    redirect($this->agent->referrer());
                    exit();
            ...
            $this->location_model->set_coordinates( $data["id"], $state, $city, $address);
            ...
            if (!empty($user)) {
                ...
                if (!$this->auth_model->is_unique_license_number($this->input->post('license_number', true),$data["id"])) {
                    $this->session->set_flashdata('license_data',  $this->input->post('license_number', true));
                    $this->session->set_flashdata('license_type',  $this->input->post('license_type', true));
                    $this->session->set_flashdata('error', "License Number exists");
                    redirect($this->agent->referrer());
                    exit();
                }
                //is slug unique
    ...
    */
    ...
    public function change_user_role_post()
    {
        ...
        $acc_type_id = $this->input->post('acc_type_id');
        if ($this->membership_model->change_user_role($user_id, $role_id, $acc_type_id)) {
            $this->session->set_flashdata('success', trans("msg_updated"));
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
        }
