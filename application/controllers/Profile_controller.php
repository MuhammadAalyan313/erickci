...
class Profile_controller extends Home_Core_Controller
{
    ...
    public function profile($slug)
    {
        ...
        $data['member_location'] = $this->settings_model->get_member_location_type()->member_location;
        if($this->auth_check){
            if( $data["user"]->id != $this->auth_user->id ){
                if( $this->license_model->get_license_restriction_setting()){
                    $allowed_licenses =json_decode($this->license_model->get_auth_user_allowed_licenses());
                    if($data['user']->license_type == Null ){
                        if(!in_array(0, $allowed_licenses )){
                            redirect($this->agent->referrer());
                            // die("Not Allowed");
                        }
                    }
                    else{
                        if(!in_array($data['user']->license_type, $allowed_licenses )){
                            redirect($this->agent->referrer());
                            // die("Not allowed");
                        }
                    }
    ...
    public function location($slug)
    {
        ...
        $data['member_location'] = $this->settings_model->get_member_location_type()->member_location;
        //die($data["user"]->show_location);
        if($data["user"]->show_location == 1 || $data['member_location'] == 1):
            $data['title'] = trans("location");
            $data['description'] = trans("location") . " - " . $this->app_name;
            $data['keywords'] = trans("location") . "," . $this->app_name;
            $data["active_tab"] = "location";
            $data['user_session'] = get_usession();
            $data["user_rating"] = calculate_user_rating($data["user"]->id);
            $data['cover_image_type'] = $this->settings_model->get_cover_image_type()->cover_image_type;
            ...
            $this->load->view('partials/_header', $data);
            $this->load->view('profile/location', $data);
            $this->load->view('partials/_footer');
        else:
          
            redirect(lang_base_url()."profile/{$data["user"]->slug}");
        endif;
    ...
    public function update_profile_post()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            $data = array(
                ...
                'phone_number' => remove_forbidden_characters_number($this->input->post('phone_number', true)),
                ...
                'online_status' => $this->input->post('online_status', true),
            );
    ...
    public function follow_unfollow_user_ajax()
    {
        //check user
        if (!$this->auth_check) {
            return 0;
