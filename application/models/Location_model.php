...
class Location_model extends CI_Model
{
    ...
    public function get_state_from_ip($state)
    {
        $this->db->select('location_states.id');
        $this->db->like('location_states.name', $state);
        $this->db->where('location_states.country_id', "230");


        $query = $this->db->get('location_states');
        return $query->row();
    }
    public function get_city_from_ip($state, $city)
    {
        $this->db->select('location_cities.id');
        $this->db->like('location_cities.name', $city);
        $this->db->where('location_cities.state_id', $state);
    ...
    public function set_location_from_ip($sate, $city)
    {

        $location = new stdClass();
        $location->country_id = 230;
        $location->state_id = !empty($sate) ? $sate : 0;
        $location->city_id = !empty($city) ? $city : 0;
        $this->session->set_userdata('mds_default_location', serialize($location));
        redirect($this->agent->referrer());
    }
    public function get_location_ip()
    {
        $ip = $this->getClientIP();
        $sess_location = $this->session->userdata('mds_default_location');
        $no_location = $this->session->userdata('no_location');
        if (!empty($sess_location)) {

            $this->get_default_location();
        } else {
            $apiToken = 'b6766700e4af55'; // Replace with your actual API token
            ...
            // $response now contains the data from the API
            if ($response) {
                $location = json_decode($response);
                $state = $this->get_state_from_ip($location->region);
                $city = $this->get_city_from_ip($state->id, $location->city);
                $this->set_location_from_ip($state->id, $city->id);
            } else {
                $this->get_default_location();
            }
        }
    ...
    public function set_coordinates($auth, $state = '', $city = '', $address = '')
    {
        $cityName = "";
        $stateName = "";
        if ($state != "" || $state != 0) {
            ...
            $stateQuery = $this->db->get('location_states');
            if ($stateQuery->num_rows() > 0) {
                $stateName = $stateQuery->row()->name;
            }
        }
        if ($city != "" || $city != 0) {
            $this->db->select('name');
            $this->db->where("id", $city);
            $cityQuery = $this->db->get('location_cities');
            if ($cityQuery->num_rows() > 0) {
                $cityName = $cityQuery->row()->name;
            }
        }
        ...
        $cords = $this->get_coordinates($auth, $stateName, $cityName, $address);
        // var_dump($stateName . " " . $cityName);
    }
    public function get_coordinates( $auth, $stateName = '', $cityName = '', $address = '')
    {
        $address = "$address $cityName $stateName United States";
        $url = "https://maps.googleapis.com/maps/api/geocode/json?address=" . urlencode($address) . "&key=AIzaSyAG03HmQeEmm_CNyhG5zT7OrIJHdmcW5nU";
