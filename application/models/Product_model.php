...
class Product_model extends CI_Model
{
    ...
    public function edit_product_details($id)
    {
        ...
        $data = array(
            ...
            'is_negotiable'=>$this->input->post('is_negotiable', true),
    ...
    public function build_query($type = "active", $compile_query = false)
    {
        ...
        if($is_not_restricted){
            $license_type = json_decode($licenseRisData->allowed_id);
        }
     
      
        $select = "products.*,
            users.username AS user_username, users.shop_name AS shop_name, users.role_id AS role_id, users.acc_type AS acc_type , users.slug AS user_slug, users.avatar AS user_logo,users.license_type,
        ...
        if($is_not_restricted){
            $this->db->where_in('users.license_type', $license_type);
        }
        if ($type == 'promoted') {
    ...
    public function filter_products($query_string_array = null, $category = null, $custom_filters = null, $user_id = null)
    {
        ...
        if (!empty($category)) {
            if($category == 'negotiable'){
                $this->db->where("products.is_negotiable",1);
                if (empty($sort)) {
        ...
        } elseif (!empty($sort) && $sort == "negotiable") {
            $this->db->where('products.is_negotiable', 1);
    ...
    public function search_products($search, $category)
    {
        if (!empty($search)) {
            ...
            if (!empty($category_ids)) {
                $this->db->where_in('products.'.$column_type, $category_ids, FALSE);
            }
            $this->db->group_start();
            foreach ($array as $item) {
                if (strlen($item) > 1) {
    ...
    public function filter_user_products($list_type)
    {
        ...
        $is_negotiable =input_get('is_negotiable');
        $q = input_get('q');
        ...
        //     $this->db->group_start();
        //     if ($is_negotiable == "out_of_stock") {
        //         $this->db->where("products.product_type = 'physical' AND products.is_negotiable =", 1);
        //     } else {
        //         $this->db->where("products.product_type = 'digital' OR products.stock >", 0);
        //     }

            if ($is_negotiable == "is_negotiable" || $is_negotiable == "non_negotiable") {
                $this->db->group_start();
                if ($is_negotiable == "is_negotiable") {
                    $this->db->where("products.product_type = 'physical' AND products.is_negotiable =", 1);
                } 
    
                else if($is_negotiable == "non_negotiable"){
                    $this->db->where("products.product_type = 'physical' AND products.is_negotiable =", 0);
                }
        
                else {
                    $this->db->where("products.product_type = 'physical' OR products.is_negotiable >=", 0);
                }
            $this->db->group_end();
        }
        ...
        if (!empty($startdate) || !empty($enddate)) {
            ...
            $this->db->where('products.created_at BETWEEN "'. $start_datetime .'" AND "'. $end_datetime .'"');
        }
        if (!empty($q)) {
            $this->db->join('product_details', 'product_details.product_id = products.id');
    ...
    */
    ...
    public function NextAd($productId){
    ...
    public function previousAd($productId){
        $this->db->where("id < ", $productId);
        $this->db->where("visibility", 1);
        $this->db->where("is_deleted", 0);
        $this->db->where("is_draft", 0);
