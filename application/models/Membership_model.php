...
class Membership_model extends CI_Model
{
    ...
    */
    ...
    {
        $data_shop = $this->request_approve_set($user_id);
    
        $this->db->where('id', $user_id);
        return $this->db->update('users', $data_shop);
    }

    public function last_approved_time($user_id){
        $this->db->select('request_action_time');
        $this->db->where('id', $user_id);
    ...
    */
    ...
    public function get_acc_type(){
        return $this->db->order_by('id')->get('acc_type')->result();
    }
    
    //change user role
    public function change_user_role($user_id, $role_id,$acc_type_id)
    {
        ...
        if (!empty($user)) {
            ...
            if (!empty($role)) {
                ...
                return $this->db->where('id', $user->id)->update('users', ['role_id' => $role->id, 'has_active_shop' => $has_active_shop,'acc_type' => $acc_type_id]);
            }
        }
        return false;
    }
    ...
    public function request_approve_set($user_id){
        ...
        if ($this->input->post('submit', true) == 1) {
            $data_shop = array(
                ...
                'request_action_time'=>$currentTime,
            );
            //update user plan
        ...
        else if ($this->input->post('submit', true) == 2) {
            $data_shop = array(
                'role_id' => 2,
                'acc_type' => 2,
                'has_active_shop' => 1,
        ...
        else if ($this->input->post('submit', true) == 3) {
            $data_shop = array(
                'role_id' => 2,
                'acc_type' => 3,
                'has_active_shop' => 1,
            ...
            $reasons = $this->input->post('decline_reason', true);
            $id = $this->input->post('id', true);
            
            $this->decline_reason_model($id,$reasons);

            return $data_shop;
        }
    }
    public function get_decline_reason($user_id){
        
        $this->db->where('user_id', $user_id);
        $retrieve_data = $this->db->get('reasons_decline_request');
        return $retrieve_data;
    }


    public function decline_reason_model($user_id,$reasons){
        
        $this->db->where('user_id', $user_id);
        $retrieve_data = $this->get_decline_reason($user_id)->row();
        $data = array(
            "user_id" => $user_id,
            "reasons" => $reasons
        );
        if($retrieve_data){
            // die(var_dump($retrieve_data));
            $this->db->where('id', $retrieve_data->id );
            $this->db->update('reasons_decline_request' ,$data);
        }
        else{
            // die("else");
            $this->db->insert('reasons_decline_request' ,$data);
        }
