...
class Settings_model extends CI_Model
{
    ...
    // cover image type setting
    public function cover_image_settings(){
        $data = array(
            'cover_image_type' => $this->input->post('cover_image_type', true),
        );
        $this->db->where('id', 1);
        return $this->db->update('cover_image', $data);
    }
    //get cover image type
    public function get_cover_image_type(){
        $this->db->select('cover_image_type'); 
        $query = $this->db->get('cover_image');
        $cover = new stdClass();

         $cover = (object)$query->row_array();
         return $cover;
    }
    //update settings
    ...
    // get_member_location_type
    public function get_member_location_type()
    {
        
        $this->db->select('member_location');
        $this->db->where('id', 1);
    ...
    public function update_preferences($form)
    {
        ...
        elseif ($form == 'member_location') {
            $data = array(
                'member_location' => $this->input->post('show_members_location', true),
    ...
    public function update_watermark_settings()
    {
        ...
        if (!empty($file_path)) {
            ...
            //upload new files
            $data['watermark_image_large'] = $this->upload_model->resize_watermark($file_path, 1920, 800);
            $data['watermark_image_mid'] = $this->upload_model->resize_watermark($file_path, 500, 300);
