...
class Review_model extends CI_Model
{
    ...
    {
        $this->db->join('users', 'users.id = reviews.user_id');
        $this->db->select('reviews.*, users.username as user_username, users.slug as user_slug, users.role_id as user_role, users.first_name as user_first_name, users.last_name as user_last_name');
    ...
    public function get_latest_reviews($limit)
    {
        ...
        $this->db->join('users', 'users.id = reviews.user_id');
        $this->db->select('reviews.*, users.username as user_username, users.role_id as user_role, users.first_name as user_first_name, users.last_name as user_last_name');
        $this->db->order_by('reviews.created_at', 'DESC');
        $this->db->limit($limit);
        $query = $this->db->get('reviews');
        return $query->result();
    ...
    */
    ...
    public function abuseReportCount(){
        return $this->db->where('is_read', 0)->count_all_results('abuse_reports');
    }

    public function marksAsRead(){
        $this->db->update('abuse_reports', array('is_read' => 1), array('is_read' => 0));
    }
