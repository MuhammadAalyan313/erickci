...
class Comment_model extends CI_Model
{
    ...
    */
    ...
    public function add_comment()
    {
        ...
        if(!empty($data['parent_id'])){
            $comment_notifi['notification_type'] = 4;
            $comment_parent = $this->comment_user($data['parent_id'])->user_id;
            $comment_notifi['commit_parent'] =  $comment_parent ;
            if( $comment_parent == $this->auth_user->id)
            $comment_notifi['deleted_by'] =  $this->auth_user->id ;
        }
        ...
        if (empty($data['parent_id'])) {
            $data['parent_id'] = 0;
            $comment_notifi['notification_type'] = 2;
        }
        if ($this->auth_check) {
        ...
        if (!empty($data['product_id']) && !empty($data['comment'])) {
            ...
            $comment_notifi['user_id'] = $this->get_product_details($data['product_id'])->user_id;
            $comment_notifi['auth_id'] = $this->auth_user->id;
            $comment_notifi['product_id'] =  $data['product_id'];
            $this->load->library('sockets');
            if( $comment_notifi['user_id'] == $this->auth_user->id){
                if(isset($comment_notifi['commit_parent'])){
                    if($comment_notifi['commit_parent'] != $this->auth_user->id ){
                        $comment_notifi['user_id'] = $comment_notifi['commit_parent'] ;
                        $this->sockets->create_connection();
                        $this->db->insert('notifications', $comment_notifi);
                   }
                }
            //    return 1;
            }
    ...
    public function comment_user($parent_id){
        $this->db->select('user_id');
    ...
    {
        $sql = "SELECT comments.*, users.shop_name AS user_shop_name, users.role_id AS user_role, users.first_name AS user_first_name, users.last_name AS user_last_name, users.slug AS user_slug, users.avatar AS user_avatar, users.user_type AS user_type
    ...
    */
    ...
    public function blogCommentsCount(){
        return $this->db->where('is_read', 0)->count_all_results('blog_comments');
    }

    public function marksAsRead(){
        $this->db->update('blog_comments', array('is_read' => 1), array('is_read' => 0));
    }
    public function productCommentsCount(){
        return $this->db->where('is_read', 0)->count_all_results('comments');
    }

    public function ProductMarksAsRead(){
        $this->db->update('comments', array('is_read' => 1), array('is_read' => 0));
    }
