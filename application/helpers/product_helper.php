...
if (!function_exists('get_product_form_data')) {
    function get_product_form_data($product,$user_id=null)
    {
        $ci =& get_instance();
        $data = new stdClass();
        ...
        if (!empty($product)) {
            ...
            if ($product->listing_type == 'sell_on_site' || $product->listing_type == 'license_key') {
                ...
                if ($ci->auth_check) {
                    ...
                    $data->button = '<button type="button" class="btn btn-md btn-block btn-product-cart" onclick="openCoversationContactButtons('.$user_id.')">' . trans("contact_seller") . '</button>';
                } else {
