...
if (!function_exists('get_profile_url_by_id')) {
    function get_profile_url_by_id($user_id)
    {
        $ci =& get_instance();
        ...
        if (!empty($user)) {
            if (!empty($user->slug)) {
                return lang_base_url() . get_route("profile", true) . $user->slug;
...
if (!function_exists('get_coupon_products_array')) {
    function get_coupon_products_array($order)
    {
...
if (!function_exists('str_username')) {
    ...
    {
        $str = trim($str);
        return url_title(convert_accented_characters($str), "_", true);
    }
}
//clean slug
...
//remove forbidden characters
if (!function_exists('remove_forbidden_characters_number')) {
    function remove_forbidden_characters_number($str)
    {
        $str = trim($str);
        $str = str_replace('#', '', $str);
        $str = str_replace(';', '', $str);
...
if (!function_exists('sent_mail')) {
    function sent_mail($email_data)
    {
        $ci =& get_instance();
       
       $ci->load->library('email');
    //    $ci->email->initialize($config);
       $ci->email->from('hello@pluglist.com', $email_data['sender_name']);
       $ci->email->to($email_data['receiver']);
       $ci->email->cc($email_data['sender_email']);
...
//get shop request count by order number
if (!function_exists('shop_request_count')) {
    function shop_request_count()
    {
        $ci =& get_instance();
        return $ci->membership_model->get_shop_opening_requests_count();
    }
}
if (!function_exists('last_approved_time_check')) {
    function last_approved_time_check($id)
    {
        $ci =& get_instance();
        return $ci->membership_model->last_approved_time($id);
    }
}
...
if (!function_exists('get_notification')) {
    ...
    {
        $ci =& get_instance();
        return $ci->notification_model->get_notification();
    }
}
if (!function_exists('new_product_count')) {
    function new_product_count()
    {
        $ci =& get_instance();
        return $ci->product_admin_model->newProductCount();
    }
}

if (!function_exists('abuse_reports_count')) {
    function abuse_reports_count()
    {
        $ci =& get_instance();
        return $ci->review_model->abuseReportCount();
    }
}


if (!function_exists('contact_message_count')) {
    function contact_message_count()
    {
        $ci =& get_instance();
        return $ci->contact_model->contactMessageCount();
    }
}
if (!function_exists('blog_comment_count')) {
    function blog_comment_count()
    {
        $ci =& get_instance();
        return $ci->comment_model->blogCommentsCount();
    }
}
if (!function_exists('product_comment_count')) {
    function product_comment_count()
    {
        $ci =& get_instance();
        return $ci->comment_model->productCommentsCount();
    }
}
if (!function_exists('comment_count')) {
    ...
    {
        $ci =& get_instance();
        return product_comment_count() + blog_comment_count();
    }
}
?>