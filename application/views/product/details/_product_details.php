        ...
        <div class="col-12">
           ...
           $tag = '';
            if($product->is_negotiable == 1):
                $tag = "<span class=\"negotiable-div\">Negotiable</span>";
            else:
                $tag = '';
            endif;
            // var_dump($product->acc_type);
            // if($product->acc_type == 1):
            //     $acc_ver  = '';
            // elseif($product->acc_type == 2):
            //     $acc_ver = '<i class="icon-verified " style= "color:orange; font-size:16px"></i>';
            // else:
            //     $acc_ver='<i class="icon-verified " style= "color:#09b1ba; font-size:16px"></i>';
            // endif;
            ?>
            <h6 class=""></h6>
            </div>
            ...
            <div class="row-custom meta">
                <div class="product-details-user">
                    <?php echo trans("by"); ?>&nbsp;
                     <div class='image-box' data-toggle = "modal" data-target="#qrPopup"> <img src="<?php echo get_user_avatar($user); ?>" alt="<?php echo get_shop_name($user); ?>" class="img-profile" id="qrpopup-btn" data-slug= "<?php echo base_url().'/profile/' . $user->slug?>" ></div>
                    <a class="link-hover" href="<?php echo generate_profile_url($product->user_slug); ?>"><?php echo character_limiter(get_shop_name_product($product), 30, '..'); ?>  </a>
                </div>
            ...
            <div class="row-custom price">
                <div id="product_details_price_container" class="d-inline">
                    <?php $this->load->view("product/details/_price", ['product' => $product, 'price' => $product->price, 'discount_rate' => $product->discount_rate, 'tag'=>$tag]); ?>
                    <?php if ($product->is_sold == 1): ?>
                        <strong class="lbl-sold"><?= trans("sold"); ?></strong><?= $tag; ?>
                    <?php endif; ?>
                </div>
                <?php $show_ask = true;
                ...
                if ($show_ask == true):?>
                    <?php if ($this->auth_check): ?>
                        ...
                        <button class="btn btn-contact-seller" onclick="createMessageWidget()"><i class="icon-envelope"></i> <?php echo trans("ask_question") ?></button>
                    <?php else: ?>
...
if ($product->listing_type == 'bidding'):
    ...
    <div class="row">
        <div class="col-12 product-add-to-cart-container">
            ...
            <?php endif; ?>
            <?php $buttton = get_product_form_data($product, $product->user_id)->button;
            if ($product->is_sold != 1 && !empty($buttton)):
                if ($this->auth_check){
                    if($this->auth_user->id != $product->user_id ){?>
                ...
                <?php 
                    ...
                    ?>
                    <div class="button-container">
                    <a href="<?= site_url().'dashboard/edit-product/'.$product->id?>" class="btn btn-md btn-block" style="background-color:white;color:#FA7348!important;border:solid 1px #FA7348">Edit Details</a>
                    </div>
            <?php   }
                }
                else{
