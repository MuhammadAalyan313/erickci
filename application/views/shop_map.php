    ...
    <section class="map-container" id="map-container">
      ...
      <script>
            ...
            geocoder.geocode({
              ...
              if (status === google.maps.GeocoderStatus.OK) {
                ...
                const cardContent = '<div style="width:100%; max-width:300px; display:flex; justify-content:center;flex-wrap:wrap;padding-bottom:5px">' +
                  ...
                  '<i ' + shop.verify + '></i></p><div style="text-align:center; font-size:12px">' + shop.license_number + '</div>' +
        ...
        async function initializeMap(userLocation) {
          ...
          var addresses = [];
          MapMarker(allShops)
          google.maps.event.addListener(map, 'center_changed', checkZoomAndRedoSearch);
          google.maps.event.addListener(map, 'zoom_changed', checkZoomAndRedoSearch);

          function checkZoomAndRedoSearch() {
            if (map.getZoom() >= 7) {
              redoSearch();
          ...
          function searchLocation() {
            ...
            // console.log("ne lng" + bounds.getNorthEast().lng())
            // console.log("ne lat" + bounds.getNorthEast().lat())
            // console.log("sw lng" + bounds.getSouthWest().lng())
            // console.log("sw lat" + bounds.getSouthWest().lat())
            ...
            formData.append('swLat', swLat);
            formData.append('neLat', neLat);
            if (neLng > 0) {
              formData.append('neLng', -neLng);
            } else {
              formData.append('neLng', neLng);
            }
            if (swLng > 0) {
              formData.append('swLng', -swLng);
            } else {
              formData.append('swLng', swLng);
            }
              ...
              if (xhrs.readyState === XMLHttpRequest.DONE) {
                ...
                if (xhrs.status === 200) {
                  ...
                  markers.forEach(marker => {
                    ...
                    document.getElementById('location-result').innerText = " shops: " + responses.data.length
          ...
          //crateing the marker function to reuse the code
        ...
        function shopListData(shop) {
          ...
          divData += ' </p>';
          if (shop.license_number != null) {
            divData += ' <span style="font-size:12px">' + shop.license_number + '</span>'
          }
        ...
        function StarsData(rate) {
          ...
          if (rate >= 1) {
            star = '<i class="icon-star"></i><i class="icon-star-o"></i><i class="icon-star-o"></i><i class="icon-star-o"></i><i class="icon-star-o"></i>'
          }
          if (rate >= 2) {
            star = '<i class="icon-star"></i><i class="icon-star"></i><i class="icon-star-o"></i><i class="icon-star-o"></i><i class="icon-star-o"></i>'
          }
          if (rate >= 3) {
            star = '<i class="icon-star"></i><i class="icon-star"></i><i class="icon-star"></i><i class="icon-star-o"></i><i class="icon-star-o"></i>'
          }
          if (rate >= 4) {
            star = '<i class="icon-star"></i><i class="icon-star"></i><i class="icon-star"></i><i class="icon-star"></i><i class="icon-star-o"></i>'
          }
          if (rate >= 5) {
        ...
        function do_map_icon() {
          ...
          directionsButton.forEach(directionBtn => {
              ...
              <div class="row m-0 align-items-center">
                ...
                <div style="flex:1;" class="shop-details p-2">
                  <div>
                    ...
                    <span style="font-size:12px"><?= $shop->license_number ?></span>
                  </div>
                  <div class="map-rating">
