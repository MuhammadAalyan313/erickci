...
<ul class="nav nav-tabs nav-tabs-horizontal nav-tabs-profile" role="tablist">
    ...
    if(is_vendor($user) || is_admin($user)):
        if($user->show_location == 1):
            ?>     
                <li class="nav-item">
                    <a class="nav-link <?php echo ($active_tab == 'location') ? 'active' : ''; ?>" href="<?php echo generate_url("location") . "/" . $user->slug; ?>"><?php echo trans("location"); ?></a>
                </li>
        <?php endif;?>
            <?php
    else:
    ...
    <?php if (($this->general_settings->reviews == 1) && is_vendor($user) && is_multi_vendor_active()): ?>
        <li class="nav-item">
