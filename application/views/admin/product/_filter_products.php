<div class="row table-filter-container">
    <div class="col-sm-12">
        ...
        <div class="item-table-filter">
        <label><?php echo trans('is_negotiable'); ?></label>
            <!-- <select name="is_negotiable" class="form-control custom-select">
                <option value="" selected><?php echo trans("all"); ?></option>
                <option value="is_negotiable" <?= input_get("is_negotiable") == "is_negotiable" ? 'selected' : ''; ?>><?php echo trans("is_negotiable"); ?></option>
                <option value="non_negotiable" <?= input_get("is_negotiable") == 'non_negotiable' ? 'selected' : ''; ?>><?php echo trans("non_negotiable"); ?></option>
            </select> -->
            <span class="form-control" style="border:none"><input style="width:auto" type="checkbox" name="is_negotiable" id="" value="is_negotiable" <?= input_get("is_negotiable") == "is_negotiable" ? 'checked' : ''; ?>></span>
            
        </div>
        <div class="item-table-filter">
        <span class="date-filter"  style="display :flex; justify-content:space-between"><label><?php echo "Date"; ?></label> <span id="clear-filter" style="color:#0000FF;cursor:pointer">Clear</span></span>
            <input type="text" class="form-control" id="daterange">
            <input type="hidden" name="startdate" value="<?= !empty(input_get("startdate")) ? input_get("startdate") : '';?>">
            <input type="hidden" name="enddate" value="<?= !empty(input_get("enddate"))  ? input_get("enddate") : '';?>">
        </div>
        <div class="item-table-filter">
