                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tr role="row">
                                ...
                                <th><?php echo "License Type"; ?></th>
                                <th><?php echo "License Number"; ?></th>
                            ...
                            <?php foreach ($users as $user):
                                ...
                                <tr>
                                    ...
                                    <td><?php $role_name = @parse_serialized_name_array($user->role_name_array, $this->selected_lang->id, true);
                                        if (!empty($role_name)):?>
                                            <strong><?= html_escape($role_name); ?></strong>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                    ...
                                    <td><?php echo html_escape($user->acc_type); ?></td>
                                    <td><?php echo html_escape($user->license_name); ?></td>
                                    <td><?php echo html_escape($user->license_number); ?></td>
                                            ...
                                            <ul class="dropdown-menu options-dropdown">
                                                <li>
                                                    <button type="button" class="btn-list-button btn-change-role" data-toggle="modal" data-target="#modalRole<?= $user->id; ?>">
                                                        <i class="fa fa-user option-icon"></i><?php echo trans('change_user_type'); ?>
                                                    </button>
                                                </li>
        ...
        <div id="modalRole<?= $user->id; ?>" class="modal fade" role="dialog">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="modal-title"><?php echo trans('change_user_type'); ?></h4>
                    </div>
                    <?php echo form_open('membership_controller/change_user_role_post'); ?>
                    <div class="modal-body">
                        <div class="form-group">
                            <h4 style="margin-top:0"><?php echo trans('user_role'); ?></h4>
                            <div class="row">
                                <input type="hidden" name="user_id" value="<?= $user->id; ?>">
                                <?php if (!empty($roles)):
                            ...
                            <h4><?php echo trans('account_type'); ?></h4>
                            <div class="row">
                                <?php foreach ($acc_type as $type):
                                        $type_name = @parse_serialized_name_array($type->name, $this->selected_lang->id, true); ?>
                                        <div class="col-sm-6 m-b-15">
                                            <input type="radio" name="acc_type_id" value="<?= $type->id; ?>" id="acc_type_id_<?= $type->id; ?>" class="square-purple" <?= $user->acc_type_id == $type->id ? 'checked' : ''; ?> required>&nbsp;&nbsp;
                                            <label for="acc_type_id_<?= $type->id; ?>" class="option-label cursor-pointer"><?= html_escape($type->name); ?></label>
                                        </div>
                                    <?php endforeach; ?>
                            </div>
                        </div>
